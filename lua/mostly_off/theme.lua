local colors = require("mostly_off.colors")
...
function M.setup()
  ...
  theme.highlights = {
    ...
    Comment = { fg = c.bg_subtle, style = options.styles.comments }, -- any comment
    ColorColumn = { bg = c.bg_very_subtle }, -- used for the columns set with 'colorcolumn'
    Conceal = { fg = c.bg_subtle }, -- placeholder characters substituted for concealed text (see 'conceallevel')
    Cursor = { fg = c.bg, bg = c.norm }, -- character under the cursor
    lCursor = { fg = c.bg, bg = c.norm }, -- the character under the cursor when |language-mapping| is used (see 'guicursor')
    CursorIM = { fg = c.norm, bg = c.bg_very_subtle }, -- like Cursor, but used when in IME mode |CursorIM|
    CursorColumn = { bg = c.bg_very_subtle }, -- Screen-column at the cursor, when 'cursorcolumn' is set.
    CursorLine = { bg = c.bg_very_subtle }, -- Screen-line at the cursor, when 'cursorline' is set.  Low-priority if foreground (ctermfg OR guifg) is not set.
    ...
    DiffAdded = { bg = c.diff.add, fg = c.norm }, -- diff mode: Added line |diff.txt|
    DiffModified = { bg = c.diff.change, fg = c.norm }, -- diff mode: Changed line |diff.txt|
    DiffRemoved = { bg = c.diff.delete, fg = c.norm }, -- diff mode: Deleted line |diff.txt|
    EndOfBuffer = { fg = c.medium_gray }, -- filler lines (~) after the end of the buffer.  By default, this is highlighted like |hl-NonText|.
    -- TermCursor  = { }, -- cursor in a focused terminal
    -- TermCursorNC= { }, -- cursor in an unfocused terminal
    ErrorMsg = { fg = c.diagnostic.error_subtle }, -- error messages on the command line
    VertSplit = { fg = c.border, bg = c.bg_very_subtle }, -- the column separating vertically split windows
    WinSeparator = { fg = c.border, bold = true, bg = c.bg_very_subtle }, -- the column separating vertically split windows
    Folded = { fg = c.medium_gray }, -- line used for closed folds
    FoldColumn = { bg = options.transparent and c.none or c.bg_subtle, fg = c.norm }, -- 'foldcolumn'
    SignColumn = { bg = options.transparent and c.none or c.bg, fg = c.red }, -- column where |signs| are displayed
    -- SignColumnSB = { bg = c.bg_sidebar, fg = c.fg_gutter }, -- column where |signs| are displayed
    -- Substitute = { bg = c.red, fg = c.black }, -- |:substitute| replacement text highlighting
    LineNr = { fg = c.fg_gutter }, -- Line number for ":number" and ":#" commands, and when 'number' or 'relativenumber' option is set.
    CursorLineNr = { fg = c.fg_gutter_cursor, bg = c.bg_very_subtle }, -- Like LineNr when 'cursorline' or 'relativenumber' is set for the cursor line.
    MatchParen = { fg = c.bg, bg = c.fg_match_paren, bold = true }, -- The character under the cursor or just before it, if it is a paired bracket, and its match. |pi_paren.txt|
    ModeMsg = { link = "MoreMsg" }, -- 'showmode' message (e.g., "-- INSERT -- ")
    MsgArea = { fg = c.norm }, -- Area for messages and cmdline
    -- MsgSeparator= { }, -- Separator for scrolled messages, `msgsep` flag of 'display'
    MoreMsg = { fg = c.medium_gray, bold = true }, -- |more-prompt|
    NonText = { fg = c.medium_gray }, -- '@' at the end of the window, characters from 'showbreak' and other characters that do not really exist in the text (e.g., ">" displayed when a double-wide character doesn't fit at the end of the line). See also |hl-EndOfBuffer|.
    Normal = { fg = c.norm, bg = options.transparent and c.none or c.bg }, -- normal text
    ...
    Pmenu = { bg = c.bg_popup, fg = c.norm }, -- Popup menu: normal item.
    PmenuSel = { bg = c.bg_pmenu_sel }, -- Popup menu: selected item.
    PmenuSbar = { bg = c.bg_subtle }, -- Popup menu: scrollbar.
    PmenuThumb = { bg = c.bg_subtle }, -- Popup menu: Thumb of the scrollbar.
    Question = { fg = c.red_subtle }, -- |hit-enter| prompt and yes/no questions
    QuickFixLine = { bg = c.bg_very_subtle }, -- Current |quickfix| item in the quickfix window. Combined with |hl-CursorLine| when the cursor is there.
    Search = { bg = c.bg_search, fg = c.norm }, -- Last search pattern highlighting (see 'hlsearch').  Also used for similar items that need to stand out.
    IncSearch = { bg = c.bg_inc_search, fg = c.fg_inc_search }, -- 'incsearch' highlighting; also used for the text replaced with ":s///c"
    CurSearch = { link = "IncSearch" },
    SpecialKey = { fg = c.fg_special_key }, -- Unprintable characters: text displayed differently from what it really is.  But not 'listchars' whitespace. |hl-Whitespace|
    ...
    TabLineSel = { fg = c.fg_tab_sel, bg = c.bg_subtle, bold = true }, -- tab pages line, active tab page label
    ...
    PreProc = { fg = c.norm }, -- (preferred) generic Preprocessor
    -- -- Include       = { }, --  preprocessor #include
    -- -- Define        = { }, --   preprocessor #define
    -- -- Macro         = { }, --    same as Define
    -- -- PreCondit     = { }, --  preprocessor #if, #else, #endif, etc.

    Type = { fg = c.norm }, -- (preferred) int, long, char, etc.
    -- -- StorageClass  = { }, -- static, register, volatile, etc.
    -- -- Structure     = { }, --  struct, union, enum, etc.
    -- -- Typedef       = { }, --  A typedef

    Special = { fg = c.norm }, -- (preferred) any special symbol
    ...
    Error = { fg = c.bg, bg = c.diagnostic.error_subtle }, -- (preferred) any erroneous construct
    Todo = { fg = c.bg, bg = c.diagnostic.todo }, -- (preferred) anything that needs extra attention; mostly the keywords TODO FIXME and XXX
    ...
    -- -- documentation.
    LspReferenceText = { bg = c.lsp_reference_text }, -- used for highlighting "text" references
    LspReferenceRead = { bg = c.lsp_reference_read }, -- used for highlighting "read" references
    LspReferenceWrite = { bg = c.lsp_reference_write }, -- used for highlighting "write" references
    ...
    DiagnosticOk = { fg = c.diagnostic.ok }, -- Used as the base highlight group. Other Diagnostic highlights link to this by default
    ...
    DiagnosticUnderlineWarn = { undercurl = true, sp = c.diagnostic.warning }, -- Used to underline "Warning" diagnostics
    ...
    DiagnosticUnderlineOk = { undercurl = true, sp = c.diagnostic.ok }, -- Used to underline "Ok" diagnostics
    --
    ...
    --
    -- GitGutter
    GitGutterAdd = { fg = c.diff_add_fg, bg = c.diff_add_bg }, -- diff mode: Added line |diff.txt|
    GitGutterChange = { fg = c.diff_info_fg, bg = c.diff_info_bg }, -- diff mode: Changed line |diff.txt|
    GitGutterDelete = { fg = c.diff_remove_fg, bg = c.diff_remove_bg }, -- diff mode: Deleted line |diff.txt|
    GitGutterChangeDelete = { fg = c.diff_info_fg, bg = c.diff_info_bg }, -- diff mode: Deleted line |diff.txt|
    ...
    --
    -- Telescope
    TelescopeBorder = { fg = c.border_highlight, bg = c.bg_float_subtle },
    TelescopeNormal = { fg = c.norm, bg = c.bg_float_subtle },
    --
    -- -- NvimTree
